  ...
  if (!isAuthed) {
    return (
      <Card className="mx-auto max-w-md p-8 text-center">
        ...
        <h2 className="mt-4 font-display text-xl font-bold">Sign in to view your analytics</h2>
        <Link
    ...
    <div className="space-y-6">
      ...
      {!metrics && isLoading ? (
        ...
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          <MetricTile label="Stories" value={formatNumber(metrics.total_writings)} icon="menu-book" />
          <MetricTile label="Chapters" value={formatNumber(metrics.total_chapters)} icon="auto-stories" />
          <MetricTile label="Followers" value={formatNumber(metrics.total_followers)} icon="person" />
          <MetricTile
            label="Reads (30d)"
            value={formatNumber(metrics.total_reads_30d)}
            icon="trending-up"
          />
        </div>
      ) : (
...
const MetricTile = ({
  ...
  label: string;
  value: string;
  icon: 'menu-book' | 'auto-stories' | 'person' | 'trending-up';
}): React.ReactElement => (
